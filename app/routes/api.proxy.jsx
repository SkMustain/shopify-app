...
export const action = async ({ request }) => {
  try {
    ...
    if (userImage) {
      // ... (Existing Vision Logic - kept intact)
    ...
    // 2. GUIDED SEARCH FLOW (WIZARD)
      ...
      // STEP 2: ROOM SELECTED -> ASK VIBE
        ...
        // Store context in title? No, just pass it in payload or visual text.
        // Actually, we can just ask the user.
        return Response.json({
          reply: `Got it, **${room}**! Now, what kind of vibe or color palette are you looking for?`,
          type: "actions",
          data: [
            { label: "Modern & Beige", payload: `flow_final:${room} Modern Beige` },
            { label: "Bold & Colorful", payload: `flow_final:${room} Bold Colorful` },
            { label: "Calm & Nature", payload: `flow_final:${room} Calm Nature` }
          ]
        }, { headers: cors?.headers || {} });
      }
      ...
      // STEP 3: FINAL TRIGGER
        ...
        // Clean the payload to get the search term
        const finalQuery = userMessage.replace("flow_final:", "").trim();
        userContext = `User completed guided flow. Request: ${finalQuery}`;
        // Let the Expert Search (below) handle it as if it were a text query
        searchQueries = []; // Will be generated by AI
        // We modify the effective user message for the AI prompt
        // But we can't change const userMessage. 
        // We'll just set a flag or let it fall through to "Standard Text" logic BUT with the override.
        shouldSearch = true;
        // Hack: We need the AI generation to use this "finalQuery" instead of "userMessage".
        // We'll assign it to a var `effectiveQuery`
      }
    }
    ...
    if (shouldSearch) {
      ...
      // Define the effective query (User input OR Guided Flow result)
      const effectiveQuery = userMessage.startsWith("flow_final:")
        ? userMessage.replace("flow_final:", "").trim()
        : userContext; // For Vastu/Text, context serves as query

      // 1. QUERY GENERATION (If not done by Vision/Vastu)
      const apiKeySetting = await prisma.appSetting.findUnique({ where: { key: "GEMINI_API_KEY" } });
      let genAI;
        ...
        if (searchQueries.length === 0) {
          try {
            ...
            if (jsonMatch) {
              ...
              searchQueries = json.searchQueries || [effectiveQuery];
